...
exports.createPayment = (req, res) => {
  ...
  if (!challan_id || !amount || !payment_method) {
    return res.status(400).json({ message: "challan_id, amount and payment_method are required" });
  }
  ...
  paymentModel.addPayment(req.body, (err, result) => {
    ...
    return res.status(201).json({ message: "Payment recorded successfully", id: result.insertId });
  });
};
